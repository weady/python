...
#--------------------第二章----------------------------------
    string.split() 方法只适用于简单的字符串分割情形,它并不允许有对个分隔符或分隔符周围不确定的空格
    re.split(r'xxx',line) 方法允许你为分割符指定多个正则模式
    str.startswith() 检查字符串的开头
    str.endswith() 检查字符串的结尾
    利用fnmatch模块提供的 fnmatch() 和fnmatchcase() 函数,可以使用shell中的通配符
    fnmatch('foo.txt', '*.txt')
    findall() 方法会搜索文本并以列表形式返回所有的匹配
    如果你想以迭代方式返回匹配，可以使用 finditer() 方法来代替
    re 模块 如果进行大量匹配和搜索操作,最好先编译正则表达 regester = re.compile(r'xxxx')
    在定义正则表达的时候,通常会利用括号去捕获分组 
    datepat = re.compile(r'(\d+)/(\d+)/(\d+)') m = datepat.match('11/27/2012') 
    m.groups() 返回一个元组,包含所有匹配的对象 datepat.findall(xxx) 返回一个列表 
    str.replace() 字符串替换
    re 模块中的re.sub('被匹配的模式','替换模式','文本内容') 被匹配的模式可以使用re.compile()进行编译好
    re.subn() 可以得到匹配的多少次
    在正则表达式中*操作符是贪婪的，因此匹配操作会查找最长的可能匹配
    通过在 * 或者 + 这样的操作符后面添加一个 ? 可以强制匹配算法改成寻找最短的可能匹配
    字符串对齐 ljust() rjust() center() 所有的方法都接受一个可选的填充字符 text.ljust(20,'=')
    format(text,'=>20s') 使用 <,> ^ 进行格式处理
    字符串中插入变量 format() 函数
    s = '{name}' has {n} messages s.format(name='xxx',n=10)
    如果要被替换的变量能在变量域中找到 format_map() vars() s.format_map(vars())
    以指定列宽格式化字符串,如果你有一些长字符串,可以利用指定的列宽将它们重新格式化
    import textwrap print(textwrap.fill(s,100))

#--------------------第三章----------------------------------
    数字的四舍五入 round(value, ndigits)
    round(1.23,1) 当一个值刚好在两个边界的中间的时候
    round 函数返回离它最近的偶数 也就是说，对1.5或者2.5的舍入运算都会得到2
    传给 round() 函数的 ndigits 参数可以是负数,这种情况下 舍入运算会作用在十位、百位、千位等上面
    a=1627731 round(a,-1) -->1627730
    利用decimal 模块进行浮动数的精度运算
    from decimal import Decimal 
        a=decimal('2.4')
    数字的格式化输出使用 format() format(1234.355,'0.2f') format(x,'^|<|>10.1f') 居中 左 右对齐
    同时指定宽度和精度的一般形式是 '[<>^]?width[,]?(.digits)?' ， 其中 width 和 digits 为整数，？代表可选部分
    二八十六进制整数 bin() oct() hex() bin(1234) --> 二进制 
    format(111,'b') b-->二进制 o-->八进制 x-->十六进制
    int('10010',2) 十进制转二进制 16 十六进制 8八进制
    大型数组运算可以使用 NumPy 模块进行操作
